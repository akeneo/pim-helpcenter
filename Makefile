...
deploy: build
	$(DOCKER_RUN) -v $${SSH_AUTH_SOCK}:/ssh-auth.sock:ro -e SSH_AUTH_SOCK=/ssh-auth.sock $(DOCKER_IMAGE_TAG) rsync -e "ssh -q -p $${PORT} -o StrictHostKeyChecking=no" -qaz --delete dist/pim/v2/ akeneo@${HOSTNAME}:/home/akeneo/pim/v2
