...
test('It replaces existing announcements from firestore to fix a typo for example.', async () => {
    await firebaseTest.firestore().collection('announcements-test').doc('update_1-optimize-weight-assets_2020-03').set({startDate: '3000-01-01'});
    const beforeTestDoc = await firebaseTest.firestore().collection('announcements-test').doc('update_1-optimize-weight-assets_2020-03').get();

    expect(beforeTestDoc.data().startDate).toStrictEqual('3000-01-01');

    await pushAnnouncements('./tests/updates/nominal_case/expected_updates.json', 'announcements-test');

    const afterTestDoc = await firebaseTest.firestore().collection('announcements-test').doc('update_1-optimize-weight-assets_2020-03').get();
    expect(afterTestDoc.data().startDate).toStrictEqual('2020-03-05');
});
